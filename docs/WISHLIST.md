...
This document records useful future features that are intentionally not current Skuld behavior.

## Stack Discovery Providers

Skuld should eventually discover adjacent runtime and exposure stacks without turning into a deployment framework or configuration editor.
...
These providers should enrich Skuld's visibility first. They should not weaken the rule that host-mutating service operations resolve through Skuld's registry and an explicit backend operation contract. nginx already follows that shape as a Linux-only explicit read-only provider; Docker and Caddy should keep the same discipline when they move out of wishlist status.
