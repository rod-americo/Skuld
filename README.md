...
Local service operation tends to drift into shell aliases, one-off scripts, and
...
Python 3.9 or newer is required. Normal CLI use has no external Python
dependencies.
...
All entrypoints dispatch through `skuld.skuld_entrypoint:main`, which selects
`skuld.skuld_macos` on macOS and `skuld.skuld_linux` elsewhere.
...
Columns can be selected per invocation, saved in user config, or supplied by
...
The nginx provider renders route visibility and `describe` enrichment. It does
not add registry targets, reload nginx, or edit nginx configuration.
...
externally tracked launchd jobs whose plist declares `StandardOutPath` or
`StandardErrorPath`.
