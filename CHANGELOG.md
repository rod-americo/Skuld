...
the direct checkout wrapper from `./skuld` to `bin/skuld`, and kept the installed console command as `skuld`.
- Reintroduced service creation and deletion for Skuld-managed user services
only, with PM2-style `start --name NAME -- COMMAND`, `delete`, `ls`, and `info` command compatibility.
...
resolution, backend command routing, stats/logs behavior, doctor findings, and entrypoint dispatch.
...
provider is enabled, and Linux `describe` now shows matched nginx routes including their source config file.
...
show a numbered column catalog and accept catalog-style column IDs such as `1 2 3`.
...
`skuld_common.py` and `skuld_registry.py`, reducing backend duplication without changing public commands.
...
systemd execution behind a dedicated adapter module while preserving CLI behavior.
...
the CLI can be installed as a standard Python console command while `bin/skuld` remains supported for direct checkout use.
- Added GitHub Actions CI for non-mutating Ubuntu/macOS validation, including
syntax, unit tests, gate/doctor checks, shell syntax, and package install verification.
...
`skuld_linux_handlers.py` and `skuld_macos_handlers.py`, leaving backend entrypoints as thin composition roots.
...
orchestration, logs command flow, and `status`/`stats`/`describe` detail command flow into `skuld_linux_commands.py` and `skuld_macos_commands.py`.
...
are rewritten only by explicit mutating commands or intentional `write_back=True` code paths.
